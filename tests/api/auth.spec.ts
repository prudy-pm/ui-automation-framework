...
test.describe('Account API', () => {
  ...
  // updateAccount only accepts PUT -- POST returns this exact 405 message.
  ...
  (accountProfiles as AccountProfile[]).forEach((profile) => {
    // Full lifecycle, deliberately stateful: each step's assertion depends on the state the step before it left.
    test(`account lifecycle: create, verify, update, verify, delete a ${profile.profile} @regression`, async ({
      ...
      // finally: a failed assertion partway through must not leak the account -- no bulk-delete tool exists.
      try {
